...
contract Exchange {
 ...
 function withdrawToken(address _token, uint256 _amount) public {
  ...
  function _trade(uint256 _orderId, address _user, address _tokenGet, uint256 _amountGet, address _tokenGive, uint256 _amountGive) internal {
    ...
    //uint256 _feeAmount1 = _amountGet.mul(feePercent1).div(100);
    //uint256 _feeAmount2 = _amountGet.mul(feePercent2).div(100);

    //tokens[_tokenGet][msg.sender] = tokens[_tokenGet][msg.sender].sub(_amountGet.add(_feeAmount));
    //tokens[_tokenGet][msg.sender] = tokens[_tokenGet][msg.sender].sub(_amountGet.add(_feeAmount1));
    //tokens[_tokenGet][msg.sender] = tokens[_tokenGet][msg.sender].sub(_amountGet.add(_feeAmount2));
    //tokens[_tokenGet][_user] = tokens[_tokenGet][_user].add(_amountGet);
    //tokens[_tokenGet][feeAccount] = tokens[_tokenGet][feeAccount].add(_feeAmount);
    //tokens[_tokenGet][feeAccount1] = tokens[_tokenGet][feeAccount1].add(_feeAmount1);
    //tokens[_tokenGet][feeAccount2] = tokens[_tokenGet][feeAccount2].add(_feeAmount2);
